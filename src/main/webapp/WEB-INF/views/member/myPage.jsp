...
$(function(){
	...
	$('#photo').change(function(){
		let my_photo = this.files[0]; //배열로 반환하기 때문에 0
		if(!my_photo){
		...
		if(my_photo.size > 1024*1024){
			...
			$('.my-photo').attr('src', photo_path); //용량제한에 걸리면 원본이미지로 되돌리기
	...
	$('#photo_submit').click(function(){
		...
		jqeury로 호출	$('#photo')[0].files[0] // jquery는 모든 데이터를 배열로 인식함 (하나밖에 없어도) 그래서 해당 태그 자원을 읽어오려면 태그[0] 파일[0] 인식
		*/
		...
		$.ajax({
			...
			data:form_data,
			dataType:'json',
			//파일 업로드할 경우에는 contenttype이랑 processData 꼭 명시해야함
...
<div class="page-main">
	...
	<div class="content-main">
		<h2>회원정보</h2>
		<div class="mypage-div"> <%-- 공간을 좌우로 나누기 위해 --%>
			<h3>프로필 사진</h3>
			<ul>
				<li>
