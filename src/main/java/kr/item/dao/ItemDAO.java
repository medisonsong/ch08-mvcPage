...
public class ItemDAO {
	...
	public void insertItem(ItemVO item)throws Exception{
		...
		try {
			...
			//SQL문 작성
			sql = "INSERT INTO zitem (item_num,name,price,quantity,photo1,photo2,detail,status) VALUES "
					+ "(zitem_seq.nextval,?,?,?,?,?,?,?)";
	...
	public int getItemCount(String keyfield, String keyword, int status)throws Exception{
		...
		try {
			...
			//SQL문 작성
			sql = "SELECT COUNT(*) FROM zitem WHERE status > ? " + sub_sql; // status:1미표시,2표시 / 관리자/사용자 모두 사용할 수 있게 >로 조건 (status>0 or 1) 
